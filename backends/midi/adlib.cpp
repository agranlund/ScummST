...
private:
	...
	void generate_samples(uint8 *buf, int len);
	void on_timer();
	...
	static void premix_proc(void *param, uint8 *buf, uint len);
};
...
MidiDriver *MidiDriver_ADLIB_create(SoundMixer *mixer) {
	debug(9, " - MidiDriver_ADLIB_create - ");
	return new MidiDriver_ADLIB(mixer);
}

// All the code brought over from IMuseAdlib

void MidiDriver_ADLIB::premix_proc(void *param, uint8 *buf, uint len) {
...
void MidiDriver_ADLIB::generate_samples(uint8 *data, int len) {
	int step;
...
void MidiDriver_ADLIB::mc_inc_stuff(AdlibVoice *voice, Struct10 *s10, Struct11 *s11) {
	...
	if (code & 1) {
		...
		case 0:
			...
			adlib_set_param(voice->_channel, 0, volume_table[lookup_table[voice->_vol_2][part->_vol_eff >> 2]]);
			break;
		case 13:
			voice->_vol_1 = s10->start_value + s11->modify_val;
			if (voice->_twochan) {
				adlib_set_param(voice->_channel, 13, volume_table[lookup_table[voice->_vol_1][part->_vol_eff >> 2]]);
			} else {
